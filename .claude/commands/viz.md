...
**触发时**，在分析摘要后追加多卡提案，格式如下：

```
⚠️ 这篇文章信息量较大，单张卡片会丢失核心价值。建议拆成 {N} 张卡片系列：
...
你想怎么做？
  A. 生成完整系列（我们逐张确认，你选风格后生成）
  B. 只生成其中某张（告诉我编号）
  C. 压缩成一张总览卡（会丢失细节，适合预览/引流）
```

**不触发时**，正常进入 Step 3，不展示此提案。

**用户选 A**：从 Card 1 开始，每张生成前先确认标题和覆盖范围，确认后再执行 Step 3→5，完成后继续下一张。所有卡片保存到 `examples/{slug}/cards/card-{N}-{slug}.html/png`。

**用户选 B**：只执行指定卡片，其他跳过。

**用户选 C**：进入 Step 3，标注「总览卡」模式，内容提炼只保留框架骨架和核心结论，不展开细节。
