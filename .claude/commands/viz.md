...
根据 `analysis.md` 的主结构类型，在下表中找到匹配风格：

| 文章主结构 | 推荐系列风格 |
|---|---|
| 层级 + 框架驱动，有强结论 | INTJ 框架演绎 |
| 对比 + 量化，有验收标准 | ESTJ 执行流程图 |
| 推导 + 条件分支，开放收口 | INTP 推导草稿 |
| 步骤 + 操作手册，明确完成 | ISFJ 步骤手册 |
| 叙事 + 论证密集 | INFJ 意义地图 |
| 行动 + 即时决策，数据驱动 | ESTP 战术简报 |

找到推荐风格后，**用自然语言**说明推荐理由——不要直接搬表格里的字段，而要结合这篇文章的实际内容说话。例如：「这篇文章以 Select/Compress/Isolate/Write 框架统领全文，有清晰的层级和强结论，INTJ 框架演绎风格的冷蓝框架容器 + 结论 bar 能让读者一眼抓住逻辑骨架。」理由里要提到文章的具体结构特点，不只是风格特征。

同时告知用户可以覆盖：「你也可以选其他风格，我可以列出全部 16 种。」
