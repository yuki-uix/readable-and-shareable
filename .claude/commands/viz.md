...
根据 `analysis.md` 的主结构类型，推荐一种 MBTI Note 风格作为整个系列的基底：

| 文章主结构 | 推荐系列风格 | 理由 |
|---|---|---|
| 层级 + 框架驱动，有强结论 | INTJ 框架演绎 | 冷蓝语义色，结论 bar，演绎层级 |
| 对比 + 量化，有验收标准 | ESTJ 执行流程图 | 责任矩阵，数据表格，操作感 |
| 推导 + 条件分支，开放收口 | INTP 推导草稿 | 边注，条件标签，问号节点 |
| 步骤 + 操作手册，明确完成 | ISFJ 步骤手册 | 编号步骤，完成条件，安全感配色 |
| 叙事 + 论证密集 | INFJ 意义地图 | 概念网络，紫色系，整体感悟 |
| 行动 + 即时决策，数据驱动 | ESTP 战术简报 | 深色主题，NOW/NEXT，场景卡片 |

说明推荐理由（一句话），并告知用户可以覆盖选择其他风格。

#### 卡片拆分方案

```
⚠️ 这篇文章信息量较大，单张卡片会丢失核心价值。建议出 {N} 张系列卡：

  推荐风格：{MBTI 风格名}（{一句话理由}）
  ── 整个系列使用同一视觉风格，保持阅读连贯性 ──
